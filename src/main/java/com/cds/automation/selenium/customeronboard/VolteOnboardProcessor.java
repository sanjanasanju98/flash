...
@Log4j2
@Component
public class VolteOnboardProcessor extends BaseClassHelper {
  ...
  @Autowired
  private CustomerAccountInput accountInput;
  
  @Autowired
  private PrepaidSalesOrder prepaidSalesOrder;
  
  
  /** The method will onboard prepaid volte customer in CRM. **/
  public void onboradPrepaidVolteCustomer(final XWPFDocument doc,final Map<String,String> data) {
    try {
      ...
      accountInput.enterPrimaryInformation(data);
      final String accountNo = accountInput.enterSecondaryInput(data);
     
      prepaidSalesOrder.validateAccountDetails(data);
      prepaidSalesOrder.createSalesOrder(data,accountNo);
      
    } catch (Exception e) {
      
      log.error("Error occurred while executing volte onboarding. {}",e.getMessage());
      final StringBuilder error = 
                          new StringBuilder("Error in volte onboarding").append(e.getMessage());
      throw new CrmUiException(error.toString(),e);
    }
  } // End of onboradPrepaidVolteCustomer method.
} // End of VolteOnboardProcessor.